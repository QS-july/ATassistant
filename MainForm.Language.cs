...
namespace HLL_ATassistant
{
    public partial class MainForm
    {
        ...
        private static readonly Dictionary<string, string> EnglishTexts = new Dictionary<string, string>
        {
            ...
            { "WarningNoFixed", "Max Range not used, max range is the largest distance in list" },
        ...
        private void ApplyLanguage()
        {
            ...
            // lblSmooth.Text = texts["SmoothLabel"];
            ...
            // if (_currentMode == Mode.Idle) lblStatus.Text = texts["StatusIdle"];
            // if (!chkUseBuiltin.Checked)
            //     lblWarning.Text = texts["WarningNoFixed"];

            // 更新误差率显示（lblErrorStats）
            UpdateErrorDisplay();

            // 更新绝对误差显示（lblMultiStatus）
